...
@section('content')
    <div class="main-content-container container-fluid px-4">
        ...
        <div class="page-header row no-gutters py-4">
            <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
                {{--<h3 class="page-title">Статусы</h3>--}}
            </div>
        </div>
        <!-- End Page Header -->
        <!-- Default Light Table -->
                    ...
                    <div class="card-body p-0 pb-3 text-center">
                        <table class="table mb-0 clients-table">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="border-0">Цвет</th>
                            ...
                            <tbody>
                                ...
                                @if(@count($row) > 0)
                                    @foreach($row as $key => $status_item)
                                        
                                        <tr>
                                            <td>{{$loop->index}}</td>
                                            <td>{{$status_item->status_name}}</td>
                                            <td>{{$status_item->status_color}}</td>
                                            <td>
                                                <div class="clients-table__actions">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="/admin/status-edit/{{$status_item->status_id}}">
                                                        ...
                                                        <button type="button" class="btn btn-white" onclick="deleteStatus({{$status_item->status_id}},this)">
    ...
    <script>
        function deleteStatus(status_id,ob){
            if (!confirm('Вы действительно хотите удалить статус №' + status_id +'?')) {
                return false;
            }
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.result == false){
                        Notify("Ошибка при удалении статуса", null, null, 'danger');
                    }
                    else{
                        Notify("Статус #" + status_id + " удален", null, null, 'success');
        ...
        function setSearchParam(){
            window.location.href = "/admin/status-list?row_count=" + $("#select-client-lines-qty").val() + "&search_word=" + $("#search_word").val();
        }
