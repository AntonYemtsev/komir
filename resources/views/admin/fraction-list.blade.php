...
@section('content')
    <div class="main-content-container container-fluid px-4">
        ...
        <div class="page-header row no-gutters py-4">
            <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
                {{--<h3 class="page-title">Фракции</h3>--}}
            </div>
        </div>
        <!-- End Page Header -->
        <!-- Default Light Table -->
                    ...
                    <div class="card-body p-0 pb-3 text-center">
                        <table class="table mb-0 clients-table">
                            ...
                            <tbody>
                                ...
                                @if(@count($row) > 0)
                                    @foreach($row as $key => $fraction_item)
                                        
                                        <tr>
                                            <td>{{$loop->index}}</td>
                                            <td>{{$fraction_item->fraction_name}}</td>
                                            <td>{{$fraction_item->brand_name}}</td>
                                            <td>
                                                <div class="clients-table__actions">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="/admin/fraction-edit/{{$fraction_item->fraction_id}}">
                                                        ...
                                                        <button type="button" class="btn btn-white" onclick="deleteFraction({{$fraction_item->fraction_id}},this)">
    ...
    <script>
        function deleteFraction(fraction_id,ob){
            if (!confirm('Вы действительно хотите удалить фракцию №' + fraction_id +'?')) {
                return false;
            }

            $.ajax({
                type: 'GET',
                url: "/admin/delete-fraction",
                data: {_token: CSRF_TOKEN, fraction_id: fraction_id},
                success: function(data){
                    if(data.result == false){
                        Notify("Ошибка при удалении фракции", null, null, 'danger');
                    }
                    else{
                        Notify("Фракция #" + fraction_id + " удалена", null, null, 'success');
        ...
        function setSearchParam(){
            window.location.href = "/admin/fraction-list?row_count=" + $("#select-client-lines-qty").val() + "&search_word=" + $("#search_word").val();
        }
