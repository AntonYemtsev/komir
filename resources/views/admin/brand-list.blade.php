...
@section('content')
                ...
                <div class="card card-small mb-4">
                    ...
                    <div class="card-body p-0 pb-3 text-center">
                        <table class="table mb-0 clients-table">
                            ...
                            <tbody>
                                @if(@count($row) > 0)
                                    @foreach($row as $key => $brand_item)
                                        <tr>
                                            <td>{{$loop->index}}</td>
                                            <td>{{$brand_item->brand_name}}</td>
                                            <td>{{$brand_item->brand_email}}</td>
                                            <td>
                                                <div class="clients-table__actions">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="/admin/brand-edit/{{$brand_item->brand_id}}">
                                                        ...
                                                        <button type="button" class="btn btn-white" onclick="deleteBrand({{$brand_item->brand_id}},this)">
    ...
    <script>
        function deleteBrand(brand_id,ob){
            if (!confirm('Вы действительно хотите удалить разрез №' + brand_id +'?')) {
                return false;
            }
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.result == false){
                        Notify("Ошибка при удалении разреза", null, null, 'danger');
                    }
                    else{
                        Notify("Разрез #" + brand_id + " удален", null, null, 'success');
        ...
        function setSearchParam(){
            window.location.href = "/admin/brand-list?row_count=" + $("#select-client-lines-qty").val() + "&search_word=" + $("#search_word").val();
        }
