...
@section('content')
    <div class="main-content-container container-fluid px-4">
        ...
        <div class="page-header row no-gutters py-4">
            <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
                {{--<h3 class="page-title">Области</h3>--}}
            </div>
        </div>
        <!-- End Page Header -->
        <!-- Default Light Table -->
                    ...
                    <div class="card-body p-0 pb-3 text-center">
                        <table class="table mb-0 clients-table">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="border-0">Цена за 1 тонну угля без НДС</th>
                                <th scope="col" class="border-0">Цена за 1 тонну угля с НДС</th>
                            ...
                            <tbody>
                                @if(@count($row) > 0)
                                    @foreach($row as $key => $region_item)
                                        <tr>
                                            <td>{{$loop->index}}</td>
                                            <td>{{$region_item->region_name}}</td>
                                            <td>{{$region_item->region_price}}</td>
                                            <td>{{$region_item->region_price_nds}}</td>
                                            <td>{{$region_item->brand_name}}</td>
                                            <td>
                                                <div class="clients-table__actions">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="/admin/region-edit/{{$region_item->region_id}}">
                                                        ...
                                                        <button type="button" class="btn btn-white" onclick="deleteRegion({{$region_item->region_id}},this)">
    ...
    <script>
        function deleteRegion(region_id,ob){
            if (!confirm('Вы действительно хотите удалить область №' + region_id +'?')) {
                return false;
            }
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.result == false){
                        Notify("Ошибка при удалении области", null, null, 'danger');
                    }
                    else{
                        Notify("Область #" + region_id + " удалена", null, null, 'success');
        ...
        function setSearchParam(){
            window.location.href = "/admin/region-list?row_count=" + $("#select-client-lines-qty").val() + "&search_word=" + $("#search_word").val();
        }
