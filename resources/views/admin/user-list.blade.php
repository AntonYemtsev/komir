...
@section('content')
                                ...
                                @if(@count($row) > 0)
                                    @foreach($row as $key => $user_item)
                                        <tr>
                                            ...
                                            <td>{{$user_item->user_surname}}</td>
                                            <td>{{$user_item->email}}</td>
                                            <td>{{$user_item->user_phone}}</td>
                                            <td>{{$user_item->role_name}}</td>
                                            <td>
                                                <div class="clients-table__actions">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="/admin/user-edit/{{$user_item->user_id}}">
                                                        ...
                                                        <button type="button" class="btn btn-white" onclick="deleteUser({{$user_item->user_id}},this)">
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.result == false){
                        Notify("Ошибка при удалении пользователя", null, null, 'danger');
                    }
                    else{
                        Notify("Пользователь #" + user_id + " удален", null, null, 'success');
        ...
        function setSearchParam(){
            window.location.href = "/admin/user-list?row_count=" + $("#select-client-lines-qty").val() + "&search_word=" + $("#search_word").val();
        }
