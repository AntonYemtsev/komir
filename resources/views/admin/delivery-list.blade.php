...
@section('content')
    <div class="main-content-container container-fluid px-4">
        ...
        <div class="page-header row no-gutters py-4">
            <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
                {{--<h3 class="page-title">Срок доставки</h3>--}}
            </div>
        </div>
        <!-- End Page Header -->
        <!-- Default Light Table -->
                            ...
                            <tbody>
                                ...
                                @if(@count($row) > 0)
                                    @foreach($row as $key => $delivery_item)
                                        ...
                                        <tr>
                                            <td>{{$loop->index}}</td>
                                            <td>{{$delivery_item->delivery_name}}</td>
                                            <td>
                                                <div class="clients-table__actions">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="/admin/delivery-edit/{{$delivery_item->delivery_id}}">
                                                        ...
                                                        <button type="button" class="btn btn-white" onclick="deleteDelivery({{$delivery_item->delivery_id}},this)">
    ...
    <script>
        function deleteDelivery(delivery_id,ob){
            if (!confirm('Вы действительно хотите удалить срок доставки №' + delivery_id +'?')) {
                return false;
            }
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.result == false){
                        Notify("Ошибка при удалении срок доставки", null, null, 'danger');
                    }
                    else{
                        Notify("Срок доставки #" + delivery_id + " удален", null, null, 'success');
        ...
        function setSearchParam(){
            window.location.href = "/admin/delivery-list?row_count=" + $("#select-client-lines-qty").val() + "&search_word=" + $("#search_word").val();
        }
